...
export default function SearchPanel({ activeNavItem }: SearchPanelProps) {
  ...
  // make fetchInitialResources stable so effect deps are satisfied
  const fetchInitialResources = useCallback(async () => {
    try {
      ...
      // defensive: ensure data exists and is an array
      const data = Array.isArray(response.data) ? response.data : response.data?.data ?? [];
      setResources(data.slice(0, 5)); // Show first 5 items initially
  ...
  }, [fetchInitialResources, initialLoad, currentPanel]);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const branchToUse = (selectedSemester === "1" || selectedSemester === "2" || selectedSemester === "4")
        ? "COMMON"
        : selectedBranch;
      const semesterToUse = (selectedSemester === "1" || selectedSemester === "2") ? "0" : selectedSemester;

      // First get branch ID
      const branchResponse = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/getmyid/branchid`, {
        params: { branchName: branchToUse },
      });

      // Then get semester ID
      const semesterResponse = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/getmyid/semesterid`, {
      ...
      const searchResponse = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/search`, {
        params: {
          type,
      ...
      setPage(1);
      setHasMore(Boolean(searchResponse.data.hasMore));
    } catch (error) {
    ...
    try {
      if (isBookmarked) {
        ...
        const res = await axios.post(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/bookmark`,
          { userId, resourceId },
          { withCredentials: true }
        );
  ...
  return (
    <div className="bg-gradient-to-br from-white via-amber-50 to-yellow-50 rounded-2xl shadow-2xl border border-amber-200 overflow-hidden backdrop-blur-sm">
      <div>
        <ResourceModal open={isResourceModalOpen} onClose={() => setIsResourceModalOpen(false)} resource={selectedResource} />
      </div>
      ...
      <div className="relative bg-gradient-to-r from-purple-500 via-blue-500 to-orange-500 px-6 py-12 text-white overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          ...
          <p className="text-lg text-yellow-100 max-w-2xl mx-auto leading-relaxed">{currentPanel.description}</p>
        </div>
      </div>
      ...
      <div className="p-4 sm:p-6 lg:p-10">
        <div className="max-w-5xl mx-auto space-y-6 sm:space-y-8">
          ...
          <div className="animate-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="relative group">
              ...
              <div className="absolute left-5 top-1/2 transform -translate-y-1/2">
                <svg className="w-6 h-6 text-gray-400 group-focus-within:text-amber-500 transition-colors duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-6 pt-6 animate-in slide-in-from-bottom-10 duration-1000">
            ...
            <button onClick={resetForm} className="px-8 py-4 border-2 border-amber-300 text-gray-700 rounded-2xl hover:bg-amber-50 hover:border-amber-400 transition-all duration-300 font-semibold text-lg shadow-lg hover:shadow-xl">
              Reset Form
            </button>
          </div>
          ...
          <div className="space-y-6 mt-12">
            {resources.length > 0 && (
              <div className="space-y-6">
                      ...
                      <div className="flex justify-between items-start gap-6">
                          ...
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="flex flex-wrap items-center gap-3">
                                {resource.subject?.subjectName && <span className="bg-amber-100 text-amber-800 px-3 py-1 rounded-full text-sm font-medium">{resource.subject.subjectName}</span>}
                                {resource.subject?.subjectCode && <span className="bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full text-sm font-medium">{resource.subject.subjectCode}</span>}
                                <span className="bg-gray-100 text-gray-700 px-3 py-1 rounded-full text-sm font-medium">{formatFileSize(resource.fileSize)}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          <button
                            ...
                            className={`p-3 rounded-xl transition-all duration-200 ${bookmarkedResourceIds.includes(resource.id) ? "bg-amber-100 text-amber-600 hover:bg-amber-200" : "bg-gray-100 text-gray-400 hover:bg-amber-100 hover:text-amber-600"}`}
                            title={bookmarkedResourceIds.includes(resource.id) ? "Remove bookmark" : "Add bookmark"}
                          >
                            <Bookmark className="size-6" />
                          </button>
                          <button onClick={() => openModal(resource)} className="p-3 bg-gradient-to-r from-amber-500 to-yellow-600 text-white rounded-xl hover:from-amber-600 hover:to-yellow-700 transition-all duration-200 shadow-lg hover:shadow-xl" title="View resource">
                ...
                {hasMore && (
                  <button onClick={loadMoreResources} className="w-full py-4 bg-gradient-to-r from-gray-100 to-gray-200 hover:from-gray-200 hover:to-gray-300 rounded-2xl text-gray-700 font-semibold transition-all duration-300 flex items-center justify-center gap-3 shadow-lg hover:shadow-xl">
            ...
            {resources.length === 0 && initialLoad && (
              <div className="text-center py-16">
                ...
                <button onClick={resetForm} className="px-6 py-3 bg-gradient-to-r from-amber-500 to-yellow-600 text-white rounded-xl hover:from-amber-600 hover:to-yellow-700 transition-all duration-200 font-medium">
                  Clear Search
                </button>
              </div>
            )}
