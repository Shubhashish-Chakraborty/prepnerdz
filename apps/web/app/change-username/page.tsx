...
export default function ChangeUsernamPage() {
    ...
    const handleUpdateUsername = async () => {
        if (!username) {
            showNotification('Please enter an username', 'error');
            return;
        }

        setLoading(true);
        try {
            const response = await axios.put(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/auth/user/update-username`, { username }, { withCredentials: true });
